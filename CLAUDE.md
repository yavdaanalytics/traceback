...
- `tests/regression/` — pins behaviors a "helpful" refactor could silently invert: the penalty sign convention (add, never subtract), the `penalty_weight` migration, exact efficiency-report phrasing, cursor vscdb NULL handling (`cursor-vscdb-null-value.test.ts`), and a source-scan guard (`security-invariants.test.ts`) that fails if any `src/**/*.ts` file calls `exec()`/`shell: true` or string-interpolates into an `execFileSync` argv element.
